...
export default function CurrentProbabilityControl(
  ...
  useEffect(() => {
    if (!useCustom && typeof marketProbability === "number") {
      currentProbability.value = Math.round(marketProbability);
    }
  }, [useCustom, marketProbability]);
    ...
    <div class="p-2 space-y-2 text-xs bg-gray-700 rounded-lg shadow-md">
      <div class="flex justify-between items-center">
        ...
        <div class="flex items-center space-x-2">
          <label for="override-prob" class="text-gray-400">Override</label>
          <input
            id="override-prob"
            type="checkbox"
            checked={useCustom}
      ...
      <input
        ...
        disabled={!useCustom}
        class="w-full h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer range-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-800 disabled:opacity-50"
      />
    </div>
  );
}
