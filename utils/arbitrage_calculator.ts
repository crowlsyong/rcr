...
export function calculateArbitrage(
  ...
  let low = pA;
  let high = pB_inv;
  let p_target = (low + high) / 2;

  for (let i = 0; i < 100; i++) {
    p_target = (low + high) / 2;
    if (p_target <= low || p_target >= high) {
      break;
    }

    const costA = Math.sqrt(kA * p_target / (1 - p_target)) - marketA.pool.NO;
    const sharesA = marketA.pool.YES -
      Math.sqrt(kA * (1 - p_target) / p_target);

    const costB = Math.sqrt(kB * p_target / (1 - p_target)) - marketB.pool.YES;
