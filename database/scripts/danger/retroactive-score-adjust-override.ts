...
const RETROACTIVE_ADDITIVE_ADJUSTMENTS: {
  ...
  "2025-06-28": { modifier: +50, description: "Partial un-ding (June 28, 2025)" },
  "2025-06-30": { modifier: +100, description: "Partial un-ding (June 29, 2025) - recorded on June 30 UTC" },
};
...
async function adjustUserCreditScoreHistoryAdditive(): Promise<void> {
  ...
  console.log(`Starting retroactive ADDITIVE credit score adjustment for User (${USER_USER_ID})...`);
  ...
  try {
    ...
    for await (const entry of iter) {
      ...
      if (!existingPoint || !existingPoint.timestamp || !existingPoint.creditScore) {
        console.warn(`Skipping malformed entry: ${JSON.stringify(entry.key)}`);
        continue;
      }

      const utcDateStringForComparison = getUtcDateString(existingPoint.timestamp);

      const adjustment = RETROACTIVE_ADDITIVE_ADJUSTMENTS[utcDateStringForComparison];
    ...
    if (entriesToProcess.length === 0) {
      console.log("No matching credit score entries found for additive adjustment in the retrieved range.");
      return;
    }

    console.log(`Found ${entriesToProcess.length} entries eligible for additive adjustment.`);
    ...
    for (const { key, value: existingPoint } of entriesToProcess) {
        ...
        const newCreditScore = Math.max(0, Math.min(1000, existingPoint.creditScore + adjustment.modifier));
    ...
    if (adjustedCount === 0) {
      console.log("No credit score entries were additively adjusted during this run.");
    } else {
      console.log(`Successfully additively adjusted ${adjustedCount} credit score entries.`);
    }
  } catch (error) {
    console.error(
