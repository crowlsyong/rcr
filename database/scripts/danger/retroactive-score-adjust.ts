...
const RETROACTIVE_ADJUSTMENTS: {
  ...
  "2025-06-30": { modifier: -200, description: "Second loan default (June 29, 2025) - recorded on June 30 UTC" },
};
...
async function adjustUserCreditScoreHistory(): Promise<void> {
  ...
  console.log(`Starting retroactive credit score adjustment for User (${USER_USER_ID})...`);
  ...
  try {
    ...
    for await (const entry of iter) {
      ...
      if (adjustment && !existingPoint.retroactiveAdjustment) {
        ...
        console.log(`Skipping already adjusted entry for ${existingPoint.username} on ${utcDateStringForComparison}.`);
      }
    }

    if (entriesToProcess.length === 0) {
      console.log("No new matching credit score entries found for adjustment in the retrieved range.");
      return;
    }

    console.log(`Found ${entriesToProcess.length} entries eligible for adjustment.`);
    ...
    for (const { key, value: existingPoint } of entriesToProcess) {
        ...
        const newCreditScore = Math.max(0, existingPoint.creditScore + adjustment.modifier);
    ...
    if (adjustedCount === 0) {
      ...
      console.log(`Successfully adjusted ${adjustedCount} credit score entries.`);
    }
  } catch (error) {
    console.error(
