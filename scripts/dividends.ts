...
// Re-defining sendManagram function locally within this file
...
> {
  ...
  try {
    const response = await fetch(`${MANAGRAM_API_BASE_URL}/v0/managram`, { // MANAGRAM_API_BASE_URL is now used here
      method: 'POST',
      headers: {
...
async function main() {
  ...
  if (DRY_RUN_MODE) {
    ...
    console.warn(
      'Mana WILL BE SENT to crowlsyong and shipcoin (50 mana each).',
    );
  } else {
    console.log(
  ...
  interface Investor {
    ...
    name: string;
    dividend: number;
    percentageOwned: number; // Stored as a decimal, e.g., 0.05 for 5%
  }
  ...
  if (DRY_RUN_MODE) {
    const dryRunUsers: Investor[] = [
      ...
      {
        name: 'shipcoin',
        userId: 'SVaU5Wfi5zLitd6Opz5s0RJC2bz1',
        dividend: 10,
        percentageOwned: 0.05,
      },
    ];
    ...
    console.log(
      `[DRY RUN] Actual mana will be sent to: ${
        investorsToProcess.map((i) => `${i.name} (${i.userId})`).join(', ')
      } with 50 mana each.`,
    );
  } else {
