...
async function fetchUserData(username: string) {
    ...
    if (!res.ok) throw new Error(`Failed to fetch ${username}: ${res.statusText}`);
    return await res.json();
}
  
// Fetch top-100 profit leaderboard
async function fetchLeaderboard() {
    const res = await fetch(`https://api.manifold.markets/v0/leaderboard?kind=profit&limit=100`);
...
function computeCreditScore(user: any, lbIndex: number): number {
    ...
    // Normalize the score, ensuring that the raw score is in the range [-1000, 1000]
    const rawScore = raw * 1000; // Assuming the raw score is in the range [-1, 1]
  
    // Now map the [-1000, 1000] range to [0, 1000]
    const normalizedScore = Math.min(Math.max((rawScore + 1000) / 2, 0), 1000); // Maps [-1000, 1000] to [0, 1000]
