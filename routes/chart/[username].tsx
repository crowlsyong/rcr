...
export const handler: Handlers<UserChartPageData> = {
  async GET(_req, ctx): Promise<Response> {
    ...
    try {
      ...
      const apiRes = await fetch(`${ctx.url.origin}/api/score?username=${username}`);
...
export default function UserChartPage({ data }: PageProps<UserChartPageData>) {
  ...
  if (data.error) {
    return (
      <div class="p-4 mx-auto max-w-screen-md mt-16"> {/* Added mt-16 for top padding */}
  ...
  return (
    ...
    <div class="pt-16 px-4 mx-auto max-w-screen-lg md:px-8">
      ...
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center"> {/* Avatar and title container */}
          {data.avatarUrl && (
            <img
              src={data.avatarUrl}
          ...
          <div>
            <h1 class="text-xl md:text-3xl font-bold text-gray-100 leading-tight"> {/* Adjusted title font size for mobile */}
              Credit Score Chart
            </h1>
            <p class="text-lg md:text-2xl text-gray-300">{data.username}</p> {/* Adjusted username font size for mobile */}
          </div>
        </div>
        {/* Current credit score number */}
        <div class="text-right"> {/* Ensure text is right-aligned */}
            <p class="text-sm md:text-lg text-gray-400">Current Score:</p> {/* Adjusted label font size */}
            <p class="text-3xl md:text-5xl font-bold text-blue-400">{data.creditScore}</p> {/* Adjusted score font size for mobile */}
        </div>
      </div>

      <div class="bg-gray-800 p-4 md:p-6 rounded-lg shadow-xl mb-6"> {/* Styled container for the chart */}
        {data.userId ? <CreditScoreChart userId={data.userId} /> : <p>Cannot load chart without user ID.</p>}
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-200"> {/* Moved details and notes below chart */}
        <div class="bg-gray-800 p-4 md:p-6 rounded-lg shadow"> {/* Styled container for details */}
          <h2 class="text-xl font-semibold mb-3 text-gray-100">Current Score Details</h2>
          <p class="text-sm"><strong>Risk Multiplier:</strong> {data.riskMultiplier}</p> {/* Kept font sizes consistent */}
        ...
        <div class="bg-gray-800 p-4 md:p-6 rounded-lg shadow"> {/* Styled container for notes */}
            <h2 class="text-xl font-semibold mb-3 text-gray-100">Notes</h2>
            <p class="text-xs text-gray-400">
                The historical data updates at most every 5 minutes. The current score displayed above reflects the latest data from Manifold Markets.
            </p>
